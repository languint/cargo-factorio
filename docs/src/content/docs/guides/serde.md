...
There is **no serde runtime** in Factorio, this is just transpile-time rewrites like in [Tracing](../tracing).
...
```toml
[dependencies]
factorio-rs = { version = "0.1.3", features = ["serde"] }
```
...
never runs serde; struct values are already Lua tables.
