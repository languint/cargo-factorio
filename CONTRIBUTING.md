...
- Full-modules use [`insta`](https://insta.rs/) snapshots under
  `crates/factorio-codegen/tests/snapshots/`. After an intentional emit change:

  ```bash
  INSTA_UPDATE=1 cargo test -p factorio-codegen
  # or: cargo insta review -p factorio-codegen
  ```

  Snapshots normalize the `-- Generated by factorio-rs@...` header to `@VERSION`
  so releases do not churn.
