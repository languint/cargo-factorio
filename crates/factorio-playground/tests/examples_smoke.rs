...
"#,
    ...
    let boot = map["lua/control/boot.lua"].as_str().unwrap();
    assert!(
        boot.contains("Phase.tick(phase)") || boot.contains("phase:tick()"),
        "expected method call for phase.tick(), got:\n{boot}"
    );
    assert!(
        boot.contains("tag == \"Running\""),
        "expected matches! to test Phase::Running, got:\n{boot}"
    );
}
