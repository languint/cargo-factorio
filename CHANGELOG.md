...
- **Release prune + library mods**: public shared-stage structs/enums
  (e.g. `factorio-rs-gui` widgets / `State`) are prune roots so
  `factorio-rs build --profile release` / `package` no longer empties the
  `require()` module surface.
